...
parameters {
  ...
  vector[Nsubjects] eta_dis_random_effect;
  vector[Nsubjects] eta_con_random_effect;
}
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[Nsubjects] eta_dis;
  vector<lower=0, upper=1>[Nsubjects] eta_con;
...
for (subject in 1:Nsubjects) {
  ...
  eta_dis[subject]   = inv_logit(population_locations[3]  + population_scales[3] * eta_dis_random_effect[subject]);
  eta_con[subject]   = inv_logit(population_locations[4]  + population_scales[4] * eta_con_random_effect[subject]);
...
//updating weights
  ...
  weights[1]=weights[1]+eta_dis[subject]*(0.5-weights[1]);
}
 else if(reward[subject,trial]==1){
  weights[1]=weights[1]+eta_con[subject]*(1-weights[1]);
}
...
model {
  ...
  eta_dis_random_effect ~ std_normal();
  eta_con_random_effect ~ std_normal();
