data {
  ...
  int<lower = 0> card_right[Nsubjects,Ntrials]; 
  int<lower = 0> first_location[Nsubjects,Ntrials]; 
  int<lower = 0> second_location[Nsubjects,Ntrials]; 
...
for (subject in 1:Nsubjects) {
  ...
  if (first_trial_in_block[subject,trial] == 1) {
      ...
      Q_keys=Q_keys_initial;
          ...
          Qnet[1]=beta_card[subject]*Q_cards[card_left[subject,trial]]+beta_key[subject]*Q_keys[first_location[subject,trial]]; //We compound the value of the card appearing on the left and the value of the left key.

          Qnet[2]=beta_card[subject]*Q_cards[card_right[subject,trial]]+beta_key[subject]*Q_keys[second_location[subject,trial]];
        
        //likelihood function
        Qcards_diff[trial,subject]= Q_cards[card_right[subject,trial]]-Q_cards[card_left[subject,trial]];
        Qkeys_diff[trial,subject] = Q_keys[second_location[subject,trial]]-Q_keys[first_location[subject,trial]];
