...
transformed data{
  ...
  Q_keys_initial = rep_vector(0, Noptions);
}
...
transformed parameters {
  ...
  vector[Noptions] Q_net;

for (subject in 1:Nsubjects) {
    for (trial in 1:Ntrials){
    Qcards_diff[trial,subject] = -9999;
    Qkeys_diff [trial,subject] = -9999;
  }
  ...
  for (trial in 1:Ntrials_per_subject[subject]){
     if(block[subject,trial]!=testfold){
  if (first_trial_in_block[subject,trial] == 1) {
      Q_cards=Q_cards_initial;
      Q_keys=Q_keys_initial;
    }
          Q_net[1]=Q_cards[card_left[subject,trial]]+omega[subject]*Q_keys[1]; //We compound the value of the card appearing on the left and the value of the left key.

          Q_net[2]=Q_cards[card_right[subject,trial]]+omega[subject]*Q_keys[2];
        ...
        Qnet_diff[trial,subject]  = Q_net[2]-Q_net[1]; //higher values of Qdiff mean higher chance to choose right option.
...
//Likelihood function per subject per trial
  ...
  for (subject in 1:Nsubjects){
    for (trial in 1:Ntrials_per_subject[subject]){
    if(block[subject,trial]!=testfold){ //fit parameters only for training set
...
log_lik=rep_matrix(0,Ntrials,Nsubjects);
  ...
  if (first_trial_in_block[subject,trial] == 1) {
      Qcards=rep_vector(0, Narms);
      Qkeys=rep_vector(0, Noptions);
    }
          Qnet[1]=Qcards[card_left[subject,trial]]+omega[subject]*Qkeys[1]; //We compound the value of the card appearing on the left and the value of the left key.

          Qnet[2]=Qcards[card_right[subject,trial]]+omega[subject]*Qkeys[2];
        log_lik[trial,subject] = bernoulli_logit_lpmf(selected_offer[subject, trial] | beta[subject] * Qnet);

 Qcards[ch_card[subject,trial]] += alpha[subject] * PE_cards[trial,subject]; //update card_value according to reward
 Qkeys[ch_key[subject,trial]] += alpha[subject] * PE_keys [trial,subject]; //update key value according to reward
