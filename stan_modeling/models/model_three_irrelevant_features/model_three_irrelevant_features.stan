...
transformed data{
  int<lower = 1> Nparameters=8; //number of parameters
...
parameters {
  ...
  vector[Nsubjects] beta_texture_random_effect;
}
...
for (subject in 1:Nsubjects) {
  ...
  beta_color  [subject] =       (population_locations[6]  + population_scales[6] * beta_color_random_effect[subject]);
  beta_shape  [subject] =       (population_locations[7]  + population_scales[7] * beta_shape_random_effect[subject]);
  beta_texture[subject] =       (population_locations[8]  + population_scales[8] * beta_texture_random_effect[subject]);
  ...
  if (first_trial_in_block[subject,trial] == 1) {
      ...
      Q_textures=Q_textures_initial;
          ...
          Qnet[1]=beta_key[subject]*Q_keys[key1[subject,trial]]+beta_color[subject]*Q_colors[color1[subject,trial]]+beta_shape[subject]*Q_shapes[shape1[subject,trial]]+beta_texture[subject]*Q_textures[texture1[subject,trial]]; //We compound all the values of bandit1

          Qnet[2]=beta_key[subject]*Q_keys[key2[subject,trial]]+beta_color[subject]*Q_colors[color2[subject,trial]]+beta_shape[subject]*Q_shapes[shape2[subject,trial]]+beta_texture[subject]*Q_textures[texture2[subject,trial]]; //We compound all the values of bandit2
...
model {
  ...
  beta_texture_random_effect ~ std_normal();
