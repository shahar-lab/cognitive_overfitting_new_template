...
transformed data{
  ...
  matrix [Nraffle,Narms]Q_key_cards_initial;
  Q_cards_initial = rep_vector(0.5, Narms);
  Q_keys_initial = rep_vector(0.5, Nraffle);
  Q_key_cards_initial = rep_matrix(0.5, Nraffle,Narms);
}
...
parameters {
  ...
  vector[Nsubjects] alpha_key_card_random_effect;
  vector[Nsubjects] beta_card_random_effect;
  vector[Nsubjects] beta_key_random_effect;
  vector[Nsubjects] beta_key_card_random_effect;
}
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[Nsubjects] alpha_key_card;
  vector                  [Nsubjects] beta_card;
  vector                  [Nsubjects] beta_key;
  vector                  [Nsubjects] beta_key_card;
  ...
  matrix                  [Ntrials,Nsubjects] Qkey_cards_diff;
  vector [Narms] Q_cards;
  vector [Nraffle] Q_keys;
  matrix [Nraffle,Narms] Q_key_cards;
  vector[Nraffle] Qnet;
...
for (subject in 1:Nsubjects) {
  ...
  alpha_key_card [subject] = inv_logit(population_locations[3]  + population_scales[3] * alpha_key_card_random_effect[subject]);
  beta_card [subject]      =          (population_locations[4]  + population_scales[4] * beta_card_random_effect[subject]);
  beta_key  [subject]      =          (population_locations[5]  + population_scales[5] * beta_key_random_effect[subject]);
  beta_key_card  [subject] =          (population_locations[6]  + population_scales[6] * beta_key_card_random_effect[subject]);
  for (trial in 1:Ntrials_per_subject[subject]){
  if (first_trial_in_block[subject,trial] == 1) {
      Q_cards=Q_cards_initial;
      Q_keys=Q_keys_initial;
      Q_key_cards=Q_key_cards_initial;
    }
          Qnet[1]=beta_card[subject]*Q_cards[card_left[subject,trial]]+beta_key[subject]*Q_keys[1]+beta_key_card[subject]*Q_key_cards[1,card_left[subject,trial]]; //We compound the value of the card appearing on the left and the value of the left key.

          Qnet[2]=beta_card[subject]*Q_cards[card_right[subject,trial]]+beta_key[subject]*Q_keys[2]+beta_key_card[subject]*Q_key_cards[2,card_right[subject,trial]];
        ...
        Qkey_cards_diff[trial,subject] = Q_key_cards[2,card_right[subject,trial]]-Q_key_cards[1,card_left[subject,trial]];
 ...
 Q_key_cards[ch_key[subject,trial],ch_card[subject,trial]] += alpha_key_card[subject] * (reward[subject,trial] - Q_key_cards[ch_key[subject,trial],ch_card[subject,trial]]); //update key_card value according to reward
  }
 //Qvalues update
...
model {
  ...
  alpha_key_card_random_effect ~ std_normal();
  beta_card_random_effect ~ std_normal();
  beta_key_random_effect ~ std_normal();
  beta_key_card_random_effect ~ std_normal();
