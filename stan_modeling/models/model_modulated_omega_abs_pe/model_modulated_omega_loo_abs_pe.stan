...
parameters {
  ...
  vector[Nsubjects] rho_dis_random_effect;
  vector[Nsubjects] rho_con_random_effect;
}
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[Nsubjects] rho_dis; 
  vector<lower=0, upper=1>[Nsubjects] rho_con; 
...
for (subject in 1:Nsubjects) {
  ...
  rho_dis[subject]   = inv_logit(population_locations[3]  + population_scales[3] * rho_dis_random_effect[subject]);
  rho_con[subject]   = inv_logit(population_locations[4]  + population_scales[4] * rho_con_random_effect[subject]);
...
if(trial!=Ntrials_per_subject[subject]){
  if(reward[subject,trial]==0){
   weight_key[trial+1,subject]==weight_key[trial,subject] + rho_dis[subject] * (abs(PE_key[subject,trial])-abs(PE_card[subject,trial]));
}
 if(reward[subject,trial]==1){
   weight_key[trial+1,subject]==weight_key[trial,subject] + rho_con[subject] * (abs(PE_key[subject,trial])-abs(PE_card[subject,trial]));
}
 if(weight_key[trial+1,subject]<0){
   weight_key[trial+1,subject]=0;
  }
}
...
model {
  ...
  rho_dis_random_effect ~ std_normal();
  rho_con_random_effect ~ std_normal();
...
if(trial!=Ntrials_per_subject[subject]){
  if(reward[subject,trial]==0){
   weight_key_g[trial+1,subject]==weight_key_g[trial,subject] + rho_dis[subject] * (abs(PE_key_g[subject,trial])-abs(PE_card_g[subject,trial]));
}
 if(reward[subject,trial]==1){
   weight_key_g[trial+1,subject]==weight_key_g[trial,subject] + rho_con[subject] * (abs(PE_key_g[subject,trial])-abs(PE_card_g[subject,trial]));
}
 if(weight_key_g[trial+1,subject]<0){
         weight_key_g[trial+1,subject]=0
  }
}
