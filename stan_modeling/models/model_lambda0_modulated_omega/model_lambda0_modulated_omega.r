...
sim.block = function(subject,parameters,cfg){ 
  ...
  for (block in 1:Nblocks){
    ...
    if(block%%2==0){
    expvalues=t(data.frame(a=rep(0.6,Ntrials_perblock),b=rep(0.7,Ntrials_perblock),c=rep(0.6,Ntrials_perblock),d=rep(0.7,Ntrials_perblock)))
    }
    else{
      expvalues=t(data.frame(a=rep(0.1,Ntrials_perblock),b=rep(0.7,Ntrials_perblock),c=rep(0.1,Ntrials_perblock),d=rep(0.7,Ntrials_perblock)))  
    }
    ...
    for (trial in 1:Ntrials_perblock){
      ...
      lambda=lambda0 + lambda1 * (diff_reliability)
