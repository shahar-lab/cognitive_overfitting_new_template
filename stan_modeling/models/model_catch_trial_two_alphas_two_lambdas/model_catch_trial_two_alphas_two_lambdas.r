...
sim.block = function(subject,parameters,cfg){ 
  ...
  beta_catch = parameters['beta_catch']
  #set initial var
  Narms              = cfg$Narms
  Nraffle            = cfg$Nraffle
  Nblocks            = cfg$Nblocks
  ...
  for (block in 1:Nblocks){
    expvalues=t(data.frame(a=rep(0.4,Ntrials_perblock),b=rep(0.6,Ntrials_perblock),c=rep(0.4,Ntrials_perblock),d=rep(0.6,Ntrials_perblock),e=rep(0.5,Ntrials_perblock),f=rep(0.5,Ntrials_perblock)))
    ...
    for (trial in 1:Ntrials_perblock){
      if (trial%%3==0){
        ...
        Qnet = Q_keys
        p= exp(beta_catch * Qnet) / sum(exp(beta_catch * Qnet)) #get prob for each action
        #players choice
