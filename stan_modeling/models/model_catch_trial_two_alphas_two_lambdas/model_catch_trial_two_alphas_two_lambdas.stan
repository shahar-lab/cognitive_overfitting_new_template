data {
  ...
  array[Nsubjects, Ntrials] int catch_trial;
...
parameters {
  ...
  vector[Nsubjects] beta_catch_random_effect;
}
transformed parameters {
  ...
  for (subject in 1 : Nsubjects) {
    ...
    lambda0[subject] = (population_locations[4]
                        + population_scales[4]
                          * lambda0_random_effect[subject]);
    
    beta_catch[subject] = (population_locations[5]
                           + population_scales[5]
                             * beta_catch_random_effect[subject]);
    ...
    for (trial in 1 : Ntrials_per_subject[subject]) {
      ...
      if (catch_trial[subject, trial] == 0) {
        ...
        Qnet[1] = Q_keys[1]; //We compound the value of the card appearing on the left and the value of the left key.
...
model {
  ...
  beta_catch_random_effect ~ std_normal();
  ...
  for (subject in 1 : Nsubjects) {
    for (trial in 1 : Ntrials_per_subject[subject]) {
      if (catch_trial[subject, trial] == 0) {
        ...
        target += bernoulli_logit_lpmf(selected_offer[subject, trial] | beta_catch[subject]
                                                                    * Qnet_diff[trial, subject]);
      }
    }
  }
